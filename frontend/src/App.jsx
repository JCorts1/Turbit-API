...
function App() {
  ...
  const fetchPowerCurve = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/turbines/${selectedTurbine}/power-curve?${params}`);
      ...
      setError("Dear user, the available data is from January 1, 2016, to March 31, 2016. Please select a date range within this period.");
      setPowerCurveData([]);
    } finally {
      setLoading(false);
    }
  ...
  const fetchStatistics = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/turbines/${selectedTurbine}/statistics?${params}`);
      if (response.ok) {
        const data = await response.json();
        setStatistics(data);
      } else {
  ...
  const CustomDot = (props) => {
    ...
    if (payload.readingCount > 100) {
      return (
        <circle cx={cx} cy={cy} r={4} fill="#00D4FF" fillOpacity={0.8} stroke="#00D4FF" strokeWidth={2} />
      );
    }
    return null;
  };
          ...
          <ResponsiveContainer width="100%" height={400}>
            ...
            >
              ...
              <XAxis
                dataKey="windSpeed"
                stroke="#8892a6"
                label={{ value: 'Wind Speed (m/s)', position: 'insideBottom', offset: -15, style: { fill: '#8892a6' } }}
              />
              <YAxis
                stroke="#8892a6"
                label={{ value: 'Power (kW)', angle: -90, position: 'insideLeft', style: { fill: '#8892a6' } }}
              />
              <Tooltip
                contentStyle={{ backgroundColor: '#0D1221', border: '1px solid #00D4FF', borderRadius: '8px', padding: '10px' }}
